...
# New output directory. Do NOT overwrite the old PhaseB directory.
FINAL_DIR=${RUN_ROOT}/phaseB_final1k_from9k_to10k
...
if [[ -e "${FINAL_DIR}" ]]; then
  echo "ERROR: FINAL_DIR already exists:"
  echo "  ${FINAL_DIR}"
  echo "This script will not delete anything. Rename or move it manually if needed."
  exit 1
fi

echo "============================================================"
echo "Resume natural_granularity_AC1 final 1k"
echo "Input checkpoint:"
echo "  ${INIT_CKPT}"
echo "Output directory:"
echo "  ${FINAL_DIR}"
echo "Expected final:"
echo "  Phase A 5k + Phase B 5k = total 10k"
echo "============================================================"
...
.venv/bin/python -m torch.distributed.run \
  ...
  "${COMMON_ARGS[@]}" \
  "dump_dir=${FINAL_DIR}" \
  "name=natural_granularity_AC1_final1k_from9k_to10k_6gpu_c00000_c00005" \
  "steps=1000" \
  "max_steps=1000" \
...
echo "  ${FINAL_CKPT_ROOT}/0000001000"
