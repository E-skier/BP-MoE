...
# Clean last complete checkpoint:
# Phase A 5k + Phase B 4k = total 9k
INIT_CKPT=${RUN_ROOT}/phaseB_to_total20k/checkpoints/0000004000

# New output directory. Do not write back into interrupted phaseB_to_total20k.
RESUME_DIR=${RUN_ROOT}/phaseB_resume_fromB4k_toB15k_total20k
...
if [[ -e "${RESUME_DIR}" ]]; then
  ...
  echo "  ${RESUME_DIR}"
  echo "This script will not delete or overwrite it."
  exit 1
fi
...
echo "  ${RESUME_DIR}"
echo "Training plan:"
echo "  Phase A 5k + previous Phase B 4k + new Phase B 11k = total 20k"
echo "============================================================"
