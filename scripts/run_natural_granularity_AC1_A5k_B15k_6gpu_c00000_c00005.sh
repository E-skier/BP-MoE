...
# ============================================================
# Natural-granularity AC1 PatchMoE training:
#   Phase A:  5,000 steps, lr=5e-5, cosine schedule
#   Phase B: 15,000 steps, lr=2e-6, constant schedule
# Combined optimizer steps: 20,000
# GPUs: 0--5
...
WARMSTART_DIR=${PROJECT_ROOT}/blt1b_warmstart/initial_weights/blt_1b_patchmoe_entropy_randominit_paired_dcp

RUN_ROOT=${PROJECT_ROOT}/blt1b_warmstart/natural_granularity_AC1_A5k_B15k_6gpu_c00000_c00005

PHASE_A_DIR=${RUN_ROOT}/phaseA_5k
PHASE_A_CKPT_ROOT=${PHASE_A_DIR}/checkpoints

PHASE_B_DIR=${RUN_ROOT}/phaseB_to_total20k
...
echo "Phase A: 5,000 steps"
echo "Phase B: 15,000 steps"
echo "Total:   20,000 optimizer steps"
echo "============================================================"
...
COMMON_ARGS=(
  ...
  model.moe_router_jitter=0.02
  model.moe_router_congestion_weight=0.0
  model.moe_router_z_loss_weight=0.001
  model.moe_router_anticollapse_loss_weight=0.01
  model.moe_router_dominance_loss_weight=0.01
  model.moe_router_dominance_threshold=0.35
...
# ============================================================
# Phase B: 15,000 additional steps = 20k total
# ============================================================

echo
echo "================= Phase B: +15k to total 20k AC1 ============"

run_torchrun \
  "${COMMON_ARGS[@]}" \
  "dump_dir=${PHASE_B_DIR}" \
  name=natural_granularity_AC1_phaseB_total20k_6gpu_c00000_c00005 \
  steps=15000 \
  max_steps=15000 \
...
echo "Keeping Phase A checkpoint for AC1 analysis:"
echo "  ${PHASE_A_CKPT_ROOT}"
...
echo "Phase A checkpoint has been kept."
echo "============================================================"
