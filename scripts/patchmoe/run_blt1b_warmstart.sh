...
# MODE=run only after the selected GPUs are free. This launcher intentionally
...
INIT_CKPT_DIR="${INIT_CKPT_DIR:-$ROOT_DIR/hf-weights/blt_1b_patchmoe_entropy_freq4_dcp}"
OUT_ROOT="${OUT_ROOT:-$ROOT_DIR/runs/blt1b_warmstart}"
RUN_NAME="${RUN_NAME:-patchmoe_blt1b_warmstart_entropy_freq4_smoke}"
...
preflight() {
  ...
  "$UV_BIN" run python -c     "from bytelatent.patchmoe_warmstart import load_patchmoe_warmstart_manifest; import json; print(json.dumps(load_patchmoe_warmstart_manifest('$INIT_CKPT_DIR'), indent=2))"
}
...
quote_cmd "${cmd[@]}"
echo "Log path if MODE=run: $RUN_DIR/pipeline.log"
