...
# This is the 49k-total checkpoint:
# Phase A 1k + Phase B first 29k + resume 19k.
INIT_CKPT=${PROJECT_ROOT}/blt1b_warmstart/entropy_randominit_A1k_B50k_6gpu_c00000_c00005/phaseB_resume_from29000_to_total50k/checkpoints/0000019000

# New output directory. Do NOT write into or delete the old resume directory.
FINAL_DIR=${PROJECT_ROOT}/blt1b_warmstart/entropy_randominit_A1k_B50k_6gpu_c00000_c00005/phaseB_final1k_from49000_to_total50k
FINAL_CKPT_ROOT=${FINAL_DIR}/checkpoints
...
if [[ ! -f "${INIT_CKPT}/complete.json" && ! -f "${INIT_CKPT}/.metadata" ]]; then
  ...
  echo "  ${INIT_CKPT}"
  echo "Expected complete.json or .metadata"
  exit 1
fi
...
if [[ -e "${FINAL_DIR}" ]]; then
  ...
  echo "  ${FINAL_DIR}"
  echo "Move it manually or change FINAL_DIR. This script will not delete anything."
  exit 1
fi

echo "============================================================"
echo "Final 1k resume for entropy-randominit"
echo "Input checkpoint, total 49k:"
echo "  ${INIT_CKPT}"
...
.venv/bin/python -m torch.distributed.run \
  ...
  "${COMMON_ARGS[@]}" \
  "dump_dir=${FINAL_DIR}" \
  "name=entropy_randominit_final1k_from49000_to_total50k_6gpu_c00000_c00005" \
  "steps=1000" \
  "max_steps=1000" \
  "checkpoint.path=${FINAL_CKPT_ROOT}" \
...
echo
echo "Final 1k finished successfully."
echo "Final checkpoint root:"
echo "  ${FINAL_CKPT_ROOT}"
