...
RUN_NAME="A1_topcell_balanced_2k_6gpu_nofused_clip0_nockpt"
RUN_DIR="$ROOT_DIR/experiments/bcfp_cpt/$RUN_NAME"
SOURCE_NAME="fineweb_edu_10bt_entropy_length_topcell_2kpf_1kpb"
...
exec env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 \
  ...
  -m bytelatent.train \
  config=apps/main/configs/bcfp_A1_clean.yaml \
  dump_dir="$RUN_DIR" \
  "name=$RUN_NAME" \
  steps=2000 \
  max_steps=2000 \
  ...
  "model.moe_pair_bias_mode=none" \
  "checkpoint.path=$RUN_DIR/checkpoints" \
  "checkpoint.init_ckpt_path=$ROOT_DIR/blt1b_warmstart/initial_weights/bcfp_pair_A1_clean_dcp" \
